...
public class VacationDateIntervalsValidator : IVacationDateIntervalsValidator
{
    public (bool IsValidated, List<string> Issues) Validate(List<VacationRequestInterval> dateIntervals)
    {
        ...
        if (!DateStartIntervalIsBeforeDateEndInterval(dateInterval: dateIntervals))
        {
            issues.Add("Date start interval is before date end interval");
        }
    ...
    private bool DateStartIntervalIsBeforeDateEndInterval(List<VacationRequestInterval> dateInterval)
    {
        return dateInterval.All(di => di.StartDate < di.EndDate);
    }
}